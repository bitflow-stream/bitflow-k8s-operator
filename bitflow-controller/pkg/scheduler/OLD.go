...
func (s *OldScheduler) SchedulePod(pod *corev1.Pod, step *bitflowv1.BitflowStep, sources []*bitflowv1.BitflowSource) (*corev1.Node, string) {
	logger := step.Log().WithField("pod", pod.Name).WithField("num-sources", len(sources))
	task := schedulingTask{OldScheduler: s, logger: logger, pod: pod, sources: sources}
...
func (s *OldScheduler) getSchedulerList(step *bitflowv1.BitflowStep) []string {
